    ...
    def process_ghost_protocol(self):
        ...
        if self.prime < 1 and self.ghostprotocol > 3:
            ...
            if current_value != self.ghostprotocollast:
                ...
                self.ghostprotocollast = current_value
        self.ghostprotocol += 0.01
        if self.ghostprotocol >= 25:
            exit()
...
def send_message(self):
    ...
    def knapsack(weights, values, max_weight):
        n = len(values)
        # Create a DP table to store the maximum value at each n-th item
        ...
        # Return the optimal value and the list of selected items
        return optimal_value, selected_items



    optimal_value, selected_items = knapsack(weights, values, max_weight)
    if optimal_value <= self.ghostprotocol * self.range:
