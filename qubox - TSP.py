...
def send_message(self):
    """
    Implement a traveling salesman problem solution using a fixed number of cities
    and predetermined distances. Uses nearest neighbor with 2-opt improvement.
    """
    # Fixed number of cities and their distances
    cities = 8  # Using 8 fixed cities
    distances = [
        [0, 10, 15, 20, 25, 30, 35, 40],
        [10, 0, 35, 25, 30, 20, 15, 45],
        [15, 35, 0, 30, 20, 25, 40, 15],
        [20, 25, 30, 0, 15, 40, 35, 20],
        [25, 30, 20, 15, 0, 35, 20, 25],
        [30, 20, 25, 40, 35, 0, 15, 30],
        [35, 15, 40, 35, 20, 15, 0, 25],
        [40, 45, 15, 20, 25, 30, 25, 0]
    ]
    ...
    def optimize_route(route, dist_matrix):
        improved = True
        best_distance = calculate_total_distance(route, dist_matrix)
        best_route = route[:]
    ...
    initial_path, initial_distance = nearest_neighbor(distances)
    
    # Optimize solution using 2-opt
    final_path, final_distance = optimize_route(initial_path, distances)
    
    # Compare result with threshold from data_array
